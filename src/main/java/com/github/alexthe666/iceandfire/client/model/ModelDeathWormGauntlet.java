    ...
    public void animate(ItemStack stack, float partialTick) {
        ...
        if (stack.getTagCompound() != null) {
            ...
            if (holder instanceof EntityLivingBase) {
                IEntityPropertiesCapability capability = InFCapabilities.getEntityPropertiesCapability((EntityLivingBase) holder);
                if (capability != null) {
                    int lungeTicks = capability.getDeathwormLungeTicks();
                    int previousLungeTicks = capability.getPreviousDeathwormLungeTicks();
                    float lungeProgress = ((float) previousLungeTicks + (lungeTicks - previousLungeTicks) * partialTick);
                    progressRotation(TopJaw, lungeProgress, (float) Math.toRadians(-30), 0, 0);
                    progressRotation(BottomJaw, lungeProgress, (float) Math.toRadians(30), 0, 0);
                    progressPosition(JawExtender, lungeProgress, 0, 0, -4);
                    progressPosition(JawExtender2, lungeProgress, 0, 0, -10);
                    progressPosition(JawExtender3, lungeProgress, 0, 0, -10);
                    progressPosition(JawExtender4, lungeProgress, 0, 0, -10);
                }
            }
        }
    }
