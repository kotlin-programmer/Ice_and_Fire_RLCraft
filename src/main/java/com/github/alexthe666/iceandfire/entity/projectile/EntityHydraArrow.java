...
public class EntityHydraArrow extends EntityArrow {
    ...
    public void onUpdate() {
        ...
        if (world.isRemote && !this.inGround) {
            ...
            IceAndFire.PROXY.spawnParticle(EnumParticle.HYDRA_BREATH, world, this.posX + (double) (this.rand.nextFloat() * this.width * 1.0F) - (double) this.width - d0 * 10.0D, this.posY + (double) (this.rand.nextFloat() * this.height) - d1 * 10.0D, this.posZ + (double) (this.rand.nextFloat() * this.width * 1.0F) - (double) this.width - d2 * 10.0D, 0.1D, 1.0D, 0.1D);
            IceAndFire.PROXY.spawnParticle(EnumParticle.HYDRA_BREATH, world, this.posX + (double) (this.rand.nextFloat() * this.width * 1.0F) - (double) this.width - d0 * 10.0D, this.posY + (double) (this.rand.nextFloat() * this.height) - d1 * 10.0D, this.posZ + (double) (this.rand.nextFloat() * this.width * 1.0F) - (double) this.width - d2 * 10.0D, 0.1D, 1.0D, 0.1D);
        }
    }
